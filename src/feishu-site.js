...
export function findFeishuArticleRoot(documentRef) {
  const candidates = [];
  const seen = new Set();
...
export async function extractFeishuArticle({
  documentRef,
  articleRoot,
  imageCache,
  readImage,
  ...
  if (!articleRoot.matches?.(".page-main")) return cleanFeishuArticleClone(articleRoot);
  const scrollContainer = findDocumentScrollContainer(documentRef, articleRoot);
  ...
  try {
    const result = await collectVirtualizedBlocks({
      ...
      renderAtCurrentPosition: async () => {
        const stableResult = await waitForStableCollection({
          wait: () => waitForRender(80),
          collect: () => collectRenderedBlocks(documentRef, blocks),
        });
        await cacheRenderedBlobImages({
          documentRef,
          collection: blocks,
          cache: imageCache,
          readImage,
  ...
  return blocks.size ? buildArticleFromBlocks(documentRef, blocks) : cleanFeishuArticleClone(articleRoot);
}

export function cleanFeishuArticleClone(articleRoot) {
...
export function collectRenderedBlocks(documentRef, collection) {
  ...
  for (const block of Array.from(blocks)) {
    ...
    const candidate = {
      ...
      type,
      clone: cleanFeishuArticleClone(block),
    };
